...
if __name__ == "__main__":
    ...
    list_of_words= get_existing_words("database/data_test.json")
    ...
    with open("database/data_test.json","r",encoding="utf-8") as file:
        data = json.load(file)
        for word in data:
            word = replace_s(word)
    ...
    # an  list to store the words themselves
    list_of_words= get_existing_words("database/gre_words_test.json")
    # get the words we have in the json file, this words are not processed, with only the word, and 4 meanings, 1 correct and 3 incorrect
    with open("database/gre_words_test.json","r",encoding="utf-8") as file:
        data = json.load(file)
        for word in data:
            word = replace_s(word)
            word_tuple = get_meaning_phone(word["word"],data)
